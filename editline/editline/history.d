...
void source_editrc()
{
  ...
  wchar_t* editrc = NULL;
  wchar_t[_EL_ENV_BUF_LEN] string;
  char[_EL_ENV_BUF_LEN] line;
  char[_EL_ENV_BUF_LEN] s1;
  char[_EL_ENV_BUF_LEN] s2;
  int d1 = -1;
  size_t n;
  FILE* handle = NULL;
  ...
  if (n) {
    if (!(editrc = malloc((n + 1) * wchar_t.sizeof))) {
      return;
    }
  ...
  if (!n) {
    ...
    */
    ...
    if (n) {
      if (!(appdata = malloc((n + 1) * wchar_t.sizeof))) {
        return;
      }
    ...
    if (!(editrc = malloc(n * wchar_t.sizeof))) {
      return;
    }
...
int using_history()
{
  ...
  */
  ...
  initialize history
  */
  memset(&_el_hs, 0, HISTORY_STATE.sizeof);
  _el_hs.entries = cast(HIST_ENTRY**)_el_alloc_array
    (size + 1, HIST_ENTRY.sizeof);
  if (!(_el_hs.entries)) {
    return -1;
  }
  _el_hs.length = 1;
...
char* add_history(char* line)
{
  ...
  if (_el_hs.length > _el_hs.size) {
    ...
    memmove(&_el_hs.entries[0], &_el_hs.entries[1],
      _el_hs.size * cast(HIST_ENTRY*).sizeof);
...
HIST_ENTRY* remove_history(int i)
{
  ...
  if (i < (_el_hs.length - 1)) {
    memmove(&_el_hs.entries[i], &_el_hs.entries[i + 1],
      (_el_hs.length - i - 1) * cast(HIST_ENTRY* ).sizeof);
  }
  _el_hs.entries[_el_hs.length - 1] =
    cast(HIST_ENTRY*)malloc(HIST_ENTRY.sizeof);
  memset(_el_hs.entries[_el_hs.length - 1], 0, HIST_ENTRY.sizeof);
...
void _el_display_history()
{
  ...
  h_len = cast(int)wcslen(_el_wide);
...
int _el_find_history_file(const char* filename, wchar_t** name)
{
  ...
  if (!filename) {
    ...
    if (!_el_mb2w(cast(char*)filename, name)) {
      return -1;
    }
  }
  if (appdata) {

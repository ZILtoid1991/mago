...
// #include <Psapi.h>
  ...
  /* SYNTAX ERROR: (19): expected ; instead of namespace */ 


 /* SYNTAX ERROR: (22): expected ; instead of < */ 

 /* SYNTAX ERROR: unexpected trailing } */ } /* SYNTAX ERROR: (24): expected <identifier> instead of ; */ 
...
HRESULT  Exec.DispatchAndContinue( Process* proc, ref const  DEBUG_EVENT  debugEvent )
{
    HRESULT  hr = S_OK;
...
HRESULT  Exec.DispatchProcessEvent( Process* proc, ref const  DEBUG_EVENT  debugEvent )
{
    _ASSERT( proc !is  null );
...
HRESULT  Exec.HandleCreateThread( Process* proc, ref const  DEBUG_EVENT  debugEvent )
{
    HRESULT          hr = S_OK;
    IMachine*       machine = proc.GetMachine();
    RefPtr<Thread>  thread;
...
bool  Exec.FoundLoaderBp( Process* proc, ref const  DEBUG_EVENT  debugEvent )
{
    if ( !proc.ReachedLoaderBp() )
    {
        if ( debugEvent.u.Exception.ExceptionRecord.ExceptionCode == STATUS_BREAKPOINT )
        {
            Address  exceptAddr = cast(Address) debugEvent.u.Exception.ExceptionRecord.ExceptionAddress;
...
HRESULT  Exec.HandleException( Process* proc, ref const  DEBUG_EVENT  debugEvent )
{
    HRESULT  hr = S_OK;
    IMachine* machine = proc.GetMachine();
...
HRESULT  Exec.HandleOutputString( Process* proc, ref const  DEBUG_EVENT  debugEvent )
{
    HRESULT          hr = S_OK;
    const  uint16_t   TotalLen = debugEvent.u.DebugString.nDebugStringLength;
    SIZE_T           bytesRead = 0;
    BOOL             bRet = FALSE;
    UniquePtr<wchar_t[ /* SYNTAX ERROR: (695): expression expected, not ] */ ]>    wstr( new  wchar_t[ TotalLen ] );
    ...
    {
        UniquePtr<char[ /* SYNTAX ERROR: (715): expression expected, not ] */ ]>   astr( new  char[ TotalLen ] );
        int      countRet = 0;
...
HRESULT  Exec.Launch( LaunchInfo* launchInfo, ref IProcess*  process )
{
    _ASSERT( mTid == GetCurrentThreadId() );
    if ( mTid != GetCurrentThreadId() )
        return  E_WRONG_THREAD;
    ...
    if ( (launchInfo.StdInput !is  null) || (launchInfo.StdOutput !is  null) || (launchInfo.StdError !is  null) )
    {
...
HRESULT  Exec.Attach( uint32_t  id, ref IProcess*  process )
{
    _ASSERT( mTid == GetCurrentThreadId() );
    if ( mTid != GetCurrentThreadId() )
        return  E_WRONG_THREAD;
    ...
    uint8_t* buffer )
{
    ...
    hr = machine.ReadMemory( cast(Address) address, length, lengthRead, lengthUnreadable, buffer );
...
static if(defined( _M_IX86 )) {
    ...
    hr = machine.ReadMemory( modBase,  dosHeader.sizeof, lenRead, lenUnread, cast(uint8_t*) &dosHeader );
    if ( FAILED( hr ) )
        return  hr;

    hr = machine.ReadMemory( modBase + dosHeader.e_lfanew,  ntHeaders32.sizeof, lenRead, lenUnread, cast(uint8_t*) &ntHeaders32 );
    if ( FAILED( hr ) )
        return  hr;
    ...
    while ( iLast >= iFirst )
    {
        ...
        hr = machine.ReadMemory( 
            pdataBase + iMid * RecordSize, RecordSize, lenRead, lenUnread, cast(uint8_t*) &midRec );
        if ( FAILED( hr ) )
            return  hr;
...
HRESULT  Exec.CreateModule( Process* proc, ref const  DEBUG_EVENT  event, ref RefPtr!(Module)  mod )
{
    _ASSERT( proc !is  null );
    ...
    newMod = new  Module( 
        mDebuggerProxy,
        cast(Address) loadInfo.lpBaseOfDll,
        imageInfo.Size,
...
HRESULT  Exec.CreateThread( Process* proc, ref const  DEBUG_EVENT  event, ref RefPtr!(Thread)  thread )
{
    _ASSERT( proc !is  null );
    ...
    newThread = new  Thread( 
        hThreadPtr.Get(),
        event.dwThreadId,
        cast(Address) createInfo.lpStartAddress,
        cast(Address) createInfo.lpThreadLocalBase );

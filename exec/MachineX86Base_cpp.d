...
import ThreadX86;
 ...
 /* SYNTAX ERROR: (20): expected ; instead of < */ 

 /* SYNTAX ERROR: unexpected trailing } */ } /* SYNTAX ERROR: (22): expected <identifier> instead of ; */ 

alias  BPAddressTable.iterator  BPIterator;
...
{
    _ASSERT( mProcess  is  null );
    _ASSERT( mhProcess  is  null );
...
void  MachineX86Base.GetPendingCallbackBP( ref Address  address )
{
    address = mPendCBAddr;
}
...
HRESULT  MachineX86Base.SuspendOtherThreads( uint  threadId )
{
...
HRESULT  MachineX86Base.ResumeOtherThreads( uint  threadId )
{
    _ASSERT( mIsolatedThread );
    ...
    ref MachineResult  result )
{
    ...
    if ( mhProcess  is  null )
        return  E_UNEXPECTED;
    _ASSERT( threadId != 0 );
    _ASSERT( exceptRec !is  null );
    _ASSERT( mCurThread !is  null );
...
HRESULT  MachineX86Base.RunAllActions( bool  cancel, ref MachineResult  result )
{
    HRESULT  hr = S_OK;
    ...
    ref MachineResult  result )
{
    ...
    default:
        _ASSERT_EXPR( false, "The notifier action is wrong."w );
        result = MacRes_HandledContinue;
        break;
    }
...
HRESULT  MachineX86Base.DispatchSingleStep( const  EXCEPTION_DEBUG_INFO* exceptRec, ref MachineResult  result )
{
...
HRESULT     MachineX86Base.DispatchBreakpoint( const  EXCEPTION_DEBUG_INFO* exceptRec, ref MachineResult  result )
{
    HRESULT  hr = S_OK;
...
HRESULT  MachineX86Base.SetStepRange( bool  stepIn, AddressRange  range )
{
    ...
    RangeStepPtr  rangeStep = RangeStepPtr( ThreadX86Base.AllocRange() );
    if ( rangeStep.Get() is  null )
        return  E_OUTOFMEMORY;
...
HRESULT  MachineX86Base.SetStepRange( bool  stepIn, ref RangeStepPtr  rangeStep )
{
    Motion  motion = stepIn ? Motion_RangeStepIn : Motion_RangeStepOver;
...
HRESULT  MachineX86Base.SetStepInstructionCore( Motion  motion, ref RangeStepPtr  rangeStep, int  notifier )
{
    _ASSERT( mhProcess !is  null );
    if ( mhProcess  is  null )
        return  E_UNEXPECTED;

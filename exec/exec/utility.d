...
auto MAX(ARG1, ARG2)(ARG1 a, ARG2 b)
{
    return (a > b ? a : b);
}
...
HRESULT GetLoadedImageInfo(HANDLE hProcess, void* dllBase, ref ImageInfo imageInfo)
{
    ...
    if (!.ReadProcessMemory(hProcess, ntHeadersAddr, &ntHeadersBuf,
            IMAGE_NT_HEADERS64.sizeof, &cActual))
    {
...
HRESULT ReadMemory(HANDLE hProcess, UINT_PTR address, uint32_t length,
        ref uint32_t lengthRead, ref uint32_t lengthUnreadable, uint8_t* buffer)
{
    assert(hProcess !is null);
    assert(buffer !is null);
    assert(length < limit_max(length));
    ...
    while (((lenReadable + lenUnreadable) < length) && (nextAddr >= address))
    {
        ...
        if ((memInfo.State != MEM_COMMIT) || (memInfo.Protect == 0)
    ...
    bRet = .ReadProcessMemory(hProcess, cast(const(void)*) address, buffer, lenToRead, &lenRead);
    if (!bRet)
        return GetLastHr();

    lengthRead = cast(uint32_t) lenRead;
    lengthUnreadable = cast(uint32_t)((lenUnreadable > (length - lengthRead))
            ? length - lengthRead : lenUnreadable);

    assert(lengthRead <= length);
    assert(lengthUnreadable <= length);
    assert((lengthRead + lengthUnreadable) <= length);
...
HRESULT SuspendProcess(Process process, ThreadControlProc suspendProc)
{
    ...
    assert(suspendProc !is null);

    // already suspended?
    uint32_t count = process.GetSuspendCount();
    assert(count >= 0 && count < limit_max(count));
    if (count > 0)
    {

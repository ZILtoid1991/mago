...
HRESULT  PathResolver.Init()
{
    ...
    mScratchPath.resize( MaxSize, '\0'w );

    return  S_OK;
}

HRESULT  PathResolver.GetFilePath( HANDLE  hProcess, HANDLE  hFile, void * baseAddr, ref std.wstring  filePath )
{
    HRESULT  hr = S_OK;
    wstring  devPath;
...
HRESULT  PathResolver.ResolveDeviceName( ref const  std.wstring  devPath, HANDLE  hFile, ref std.wstring  filePath )
{
    return  DriveResolveDeviceName( devPath, hFile, filePath );
}

HRESULT  PathResolver.DriveResolveDeviceName( ref const  std.wstring  devPath, HANDLE  hFile, ref std.wstring  filePath )
{
    ...
    for ( int  i = 0; i < 26; i++ )
    {
        ...
        const(wchar_t)   DriveName[3] = [ 'A'w + wchar_t( i ), ':'w, 0 ];
        DWORD            nameLen = 0;
...
HRESULT  PathResolver.GetModulePath( HANDLE  hProcess, HMODULE  hMod, ref std.wstring  path )
{
    HRESULT  hr = S_OK;
    ...
    for ( ; ; )
    {
        ...
        nRet = GetModuleFileNameEx( hProcess, hMod, &mScratchPath[0], cast(DWORD)( mScratchPath.size() ) );
...
HRESULT  PathResolver.GetProcessModulePath( HANDLE  hProcess, ref std.wstring  path )
{
    HRESULT  hr = S_OK;
    ...
    for ( ; ; )
    {
        ...
        nRet = GetProcessImageFileName( hProcess, &mScratchPath[0], cast(DWORD) mScratchPath.size() );
        if ( nRet == 0 )
        {
...
HRESULT  PathResolver.QueryDosDeviceWithScratch( const(wchar_t) * driveName )
{
    ...
    for ( ; ; )
    {
        totalLen = QueryDosDevice( driveName, &mScratchPath.at( 0 ), cast(DWORD)( mScratchPath.size() ) );
        if ( totalLen != 0 )
            break;
...
bool     PathResolver.ExpandScratchPath()
{
    ...
    mScratchPath.resize( newSize, '\0'w );

...
namespace  Mago
{
        ...
        CONTEXT_INFO*       pInfo ) 
    {
        ...
        if ( (dwFields & CIF_MODULEURL) != 0 )
        {
            if ( mModule !is  null )
            {
                ...
                if ( pInfo.bstrModuleUrl !is  null )
        ...
        if ( (dwFields & CIF_ADDRESSOFFSET) != 0 )
        {
            ...
            {
                wchar_t  offStr[MaxAddrStringLength + 1] = ""w;
                swprintf_s( offStr, MaxAddrStringLength, "+0x%x"w, offset );

                pInfo.bstrAddressOffset = SysAllocString( offStr );

                if ( pInfo.bstrAddressOffset !is  null )
        ...
        IDebugMemoryContext2** ppMemCxt ) 
    {
        ...
        Address64  newAddr = mAddr + cast(Address64) dwCount;
    ...
    {
        if ( rgpMemoryContextSet  is  null )
            return  E_INVALIDARG;
        if ( dwMemoryContextSetLen == 0 )
            return  E_INVALIDARG;
        if ( pdwMemoryContext  is  null )
            return  E_INVALIDARG;

        HRESULT  hr = S_OK;

        for ( int  i = 0; i < cast(int) dwMemoryContextSetLen; i++ )
        {
            CComPtr<IMagoMemoryContext> magoCode;
            Address64    otherAddr = 0;
            bool         result = false;

            hr = rgpMemoryContextSet[i].QueryInterface( __uuidof( IMagoMemoryContext ), cast(void **) &magoCode );
            if ( FAILED( hr ) )
            ...
            case  CONTEXT_SAME_SCOPE: 
            {
                ...
                if( magoCode.GetScope( funcSH, blockSH, module ) == S_OK )
                {
                    ...
                    {
                    case  CONTEXT_SAME_SCOPE: 
                        result = memcmp( &mBlockSH.back(), &blockSH,  blockSH.sizeof ) == 0;
                        break;
                    case  CONTEXT_SAME_FUNCTION:
                        result = memcmp( &mFuncSH, &funcSH,  mFuncSH.sizeof ) == 0;
                        break;
                    case  CONTEXT_SAME_MODULE:
    ...
    HRESULT  CodeContext.GetAddress( ref Address64  addr )
    {
        addr = mAddr;
        return  S_OK;
    }

    HRESULT  CodeContext.GetScope( ref MagoST.SymHandle  funcSH, ref MagoST.SymHandle  blockSH, ref RefPtr!(Module)  module )
    {
        FindFunction();
        funcSH = mFuncSH;
    ...
    HRESULT  CodeContext.FindFunction()
    {
        ...
        hr = session.FindGlobalSymbolByAddr( mAddr, symHandle, sec, offset, symOff );
        if ( FAILED( hr ) )
            return  hr;
    ...
    bool  CodeContext.GetAddressOffset( ref uint32_t  offset )
    {
        HRESULT  hr = S_OK;
        MagoST.SymInfoData          infoData = { 0 };
        MagoST.ISymbolInfo*        symInfo = null;
        ...
        offset = cast(uint32_t) ( mAddr - addr );
        return  true;
    }
...
static if(0) {
        ...
        session.FindLine(
 /* SYNTAX ERROR: (442): expression expected, not } */ }
        return  pos;
    }
}

...
const(wchar_t) ExceptionKeyVSSubPath[] = "\\AD7Metrics\\Exception"w;
...
static  bool  GetCategoryPath( const(wchar_t) * rootPath, ref const  GUID  category, wchar_t * fullPath, int  fullPathLen )
{
    _ASSERT( rootPath !is  null );
    _ASSERT( fullPath !is  null );

    wchar_t          guidStr[GuidLength + 1] = ""w;
    ...
    if ( err != 0 )
        return  false;
    err = wcscat_s( fullPath, fullPathLen, "\\"w );
    if ( err != 0 )
        return  false;
...
namespace  Mago
{
    ...
    bool  EngineExceptionTable.LoadDefaultExceptionCategory( 
        ...
        ref const  GUID  category )
    {
        _ASSERT( pszRegistryRoot !is  null );

        wchar_t          path[MAX_PATH] = ""w;
        ...
        int  levelsToRecurse )
    {
        ...
        {
            LONG             ret = ERROR_SUCCESS;
            wchar_t          level1Name[MaxKeyLength + 1] = ""w;
    ...
    bool  EngineExceptionTable.LoadDefaultException( const(wchar_t) * keyName, HKEY  hKey, ref const  GUID  category )
    {
        if ( (keyName  is  null) || (keyName[0] == '\0'w) )
            return  false;
        ...
        DWORD  type = 0;
        DWORD  stateSize =  excInfo.dwState.sizeof;
        DWORD  codeSize =  excInfo.dwCode.sizeof;
        CComBSTR  keyNameBstr = keyName;

        ret = RegQueryValueEx( hKey, "State"w, null, &type, cast(BYTE*) &excInfo.dwState, &stateSize );
        if ( ret != ERROR_SUCCESS )
            return  false;
        if ( type != REG_DWORD )
            return  false;

        ret = RegQueryValueEx( hKey, "Code"w, null, &type, cast(BYTE*) &excInfo.dwCode, &codeSize );
        if ( ret != ERROR_SUCCESS )
            return  false;
        if ( type != REG_DWORD )
            return  false;

        if ( keyNameBstr  is  null )
            return  false;
    ...
    bool  EngineExceptionTable.IsSupportedCategory( ref const  GUID  category )
    {
    ...
    bool  EngineExceptionTable.FindExceptionInfo( ref const  GUID  guid, DWORD  code, ref ExceptionInfo  excInfo )
    {
    ...
    bool  EngineExceptionTable.FindExceptionInfo( ref const  GUID  guid, LPCOLESTR  name, ref ExceptionInfo  excInfo )
    {

...
// #include <MagoCVConst.h>
  ...
  /* SYNTAX ERROR: (14): expected ; instead of namespace */ 
...
//----------------------------------------------------------------------------
    ...
    this.CVDecl( 
        ...
        ref const  MagoST.SymInfoData  infoData, 
        MagoST.ISymbolInfo* symInfo )
    {   mRefCount = ( 0 );
            mSymStore = ( symStore );
        _ASSERT( symStore !is  null );
        _ASSERT( symInfo !is  null );
    ...
    bool  CVDecl.GetAddress( ref MagoEE.Address  addr )
    {
        HRESULT  hr = S_OK;
    ...
    bool  CVDecl.GetOffset( ref int  offset )
    {
        return  mSymInfo.GetOffset( offset );
    }

    bool  CVDecl.GetSize( ref uint32_t  size )
    {
        return  false;
    }

    bool  CVDecl.GetBackingTy( ref MagoEE.ENUMTY  ty )
    {
        return  false;
    }

    bool  CVDecl.GetUdtKind( ref MagoEE.UdtKind  kind )
    {
    ...
    bool  CVDecl.GetBaseClassOffset( Declaration* baseClass, ref int  offset )
    {
        return  false;
    }

    bool  CVDecl.GetVTableShape( ref Declaration*  decl )
    {
    ...
    HRESULT  CVDecl.FindObject( const(wchar_t) * name, ref Declaration*  decl )
    {
        return  E_NOT_FOUND;
    }

    bool  CVDecl.EnumMembers( ref MagoEE.IEnumDeclarationMembers*  members )
    {
        return  false;
    }

    HRESULT  CVDecl.FindObjectByValue( uint64_t  intVal, ref Declaration*  decl )
    {
        return  E_NOT_FOUND;
    }
...
//----------------------------------------------------------------------------
    ...
    bool  GeneralCVDecl.GetType( ref MagoEE.Type*  type )
    {
        if ( mType  is  null )
            return  false;
...
//----------------------------------------------------------------------------
    ...
    bool  FunctionCVDecl.GetAddress( ref MagoEE.Address  addr )
    {
        uint32_t     offset = 0;
        uint16_t     sec = 0;
        ...
        if ( mSymInfo.GetClass( classIndex ) )
        {
            ...
            if( mSession.GetTypeFromTypeIndex( classIndex, handle ) )
            {
                ...
                if( mSession.GetTypeInfo( handle, infoData, classSym ) == S_OK )
                {
                    ...
                    if( mSymInfo.GetName( name ) && classSym.GetName( className ) )
                    {
                        ...
                        auto  fnTest =  /* SYNTAX ERROR: (331): expression expected, not [ */ [&](TypeIndex  type) 
                        {
         ...
         /* SYNTAX ERROR: (345): expected <identifier> instead of if */ 
            
         /* SYNTAX ERROR: (347): expected <identifier> instead of if */ 
            

        addr = mSession.GetVAFromSecOffset( sec, offset );
         /* SYNTAX ERROR: (351): expected <identifier> instead of if */ 
            

         /* SYNTAX ERROR: (354): expected <identifier> instead of return */ 
    }
...
//----------------------------------------------------------------------------
    ...
    bool  TypeCVDecl.GetType( ref MagoEE.Type*  type )
    {
        HRESULT  hr = S_OK;
        MagoST.SymTag   tag = MagoST.SymTagNull;
    ...
    bool  TypeCVDecl.GetSize( ref uint32_t  size )
    {
        return  mSymInfo.GetLength( size );
    }

    bool  TypeCVDecl.GetBackingTy( ref MagoEE.ENUMTY  ty )
    {
        HRESULT                  hr = S_OK;
        MagoST.SymTag           tag = MagoST.SymTagNull;
        DWORD                    baseTypeId = 0;
    ...
    bool  TypeCVDecl.GetBaseClassOffset( Declaration* baseClass, ref int  offset )
    {
        MagoEE.UdtKind  kind;

        if ( (baseClass  is  null) || !baseClass.IsType()
        ...
        if ( (name16  is  null) || (baseName16  is  null) )
            return  false;
        ...
        )
    {
        ...
        for ( uint16_t  i = 0; /*i < fieldCount*/; i++ )
        {
            ...
            if ( !(this. /* SYNTAX ERROR: (599): expected <identifier> instead of * */ *functor)( memberInfo, classInfo, context ) )
                break;
        }
    }
    ...
    bool  TypeCVDecl.FindClassInList(
        ...
        void * context )
    {
        _ASSERT( memberInfo !is  null );
        _ASSERT( classInfo !is  null );
        _ASSERT( context !is  null );

        FindBaseClassParams*    params = cast(FindBaseClassParams*) context;
        SymString                className;
    ...
    HRESULT  TypeCVDecl.FindObject( const(wchar_t) * name, ref Declaration*  decl )
    {
        HRESULT              hr = S_OK;
        ...
        else
        {
            ...
            if ( decl && decl.IsFunction() )
            {
                if (!decl.IsStaticFunction())
                {
                    ...
                    (cast(FunctionCVDecl*)decl).SetType( dgtype );
                }
            }
        }
    ...
    HRESULT  TypeCVDecl.FindObjectByValue( uint64_t  intVal, ref Declaration*  decl )
    {
        HRESULT              hr = S_OK;
        MagoST.TypeHandle   childTH = { 0 };
        MagoST.TypeIndex    flistIndex = 0;
    ...
    bool  TypeCVDecl.EnumMembers( ref MagoEE.IEnumDeclarationMembers*  members )
    {
...
//----------------------------------------------------------------------------
    ...
    bool  TypeCVDeclMembers.Next( ref MagoEE.Declaration*  decl )
    {
        HRESULT              hr = S_OK;
        MagoST.TypeHandle   childTH = { 0 };
    ...
    bool  TypeCVDeclMembers.Reset()
    {
        ...
        mIndex = 0;
        mCount = 0;
        memset( &mListScope, 0,  mListScope.sizeof );
    ...
    int  TypeCVDeclMembers.NextMember( ref MagoST.TypeScope  scope, ref MagoST.TypeHandle  memberTH )
    {
        MagoST.SymInfoData      infoData;
        MagoST.ISymbolInfo*    symInfo = null;
        MagoST.SymTag           tag = MagoST.SymTagNull;
...
//----------------------------------------------------------------------------
    ...
    this.ClassRefDecl( Declaration* decl, MagoEE.ITypeEnv* typeEnv )
    {   mRefCount = ( 0 );
            mOrigDecl = ( decl );
            mTypeEnv = ( typeEnv );
    }
    ...
    bool  ClassRefDecl.GetType( ref MagoEE.Type*  type )
    {
    ...
    bool  ClassRefDecl.GetAddress( ref MagoEE.Address  addr )
    {
        return  false;
    }

    bool  ClassRefDecl.GetOffset( ref int  offset )
    {
        return  false;
    }

    bool  ClassRefDecl.GetSize( ref uint32_t  size )
    {
        return  false;
    }

    bool  ClassRefDecl.GetBackingTy( ref MagoEE.ENUMTY  ty )
    {
        return  false;
    }

    bool  ClassRefDecl.GetUdtKind( ref MagoEE.UdtKind  kind )
    {
        return  false;
    }

    bool  ClassRefDecl.GetBaseClassOffset( Declaration* baseClass, ref int  offset )
    {
        return  false;
    }

    bool  ClassRefDecl.GetVTableShape( ref Declaration*  decl )
    {
        return  false;
    }
    ...
    HRESULT  ClassRefDecl.FindObject( const(wchar_t) * name, ref Declaration*  decl )
    {
        return  E_NOT_FOUND;
    }

    bool  ClassRefDecl.EnumMembers( ref MagoEE.IEnumDeclarationMembers*  members )
    {
        return  false;
    }

    HRESULT  ClassRefDecl.FindObjectByValue( uint64_t  intVal, ref Declaration*  decl )
    {
        return  E_NOT_FOUND;
    }
...
//----------------------------------------------------------------------------
    ...
    {
    public:
        bool  GetName( ref SymString  name ) { return  false; }
        bool  GetRegister( ref uint32_t  reg ) { return  false; }
        bool  GetLength( ref uint32_t  length ) { return  false; }
        bool  GetDataKind( ref DataKind  dataKind ) { return  false; }
        bool  GetLocation( ref LocationType  locType ) { return  false; }

        bool  GetType( ref TypeIndex  index ) { return  false; }
        bool  GetAddressOffset( ref uint32_t  offset ) { return  false; }
        bool  GetAddressSegment( ref uint16_t  segment ) { return  false; }
        bool  GetOffset( ref int32_t  offset ) { return  false; }
        bool  GetRegisterCount( ref uint8_t  count ) { return  false; }
        bool  GetRegisters( ref uint8_t*  regs ) { return  false; }
        bool  GetUdtKind( ref UdtKind  udtKind ) { return  false; }
        bool  GetValue( ref Variant  value ) { return  false; }

static if(1) {
        bool  GetDebugStart( ref uint32_t  start ) { return  false; }
        bool  GetDebugEnd( ref uint32_t  end ) { return  false; }
        //virtual bool GetProcFlags( CV_PROCFLAGS& flags ) { return false; }
        bool  GetProcFlags( ref uint8_t  flags ) { return  false; }
        bool  GetThunkOrdinal( ref uint8_t  ordinal ) { return  false; }
        bool  GetBasicType( ref DWORD  basicType ) { return  false; }
        bool  GetIndexType( ref TypeIndex  index ) { return  false; }
        bool  GetCount( ref uint32_t  count ) { return  false; }
        bool  GetFieldCount( ref uint16_t  count ) { return  false; }
        bool  GetFieldList( ref TypeIndex  index ) { return  false; }
        bool  GetProperties( ref uint16_t  props ) { return  false; }
        bool  GetDerivedList( ref TypeIndex  index ) { return  false; }
        bool  GetVShape( ref TypeIndex  index ) { return  false; }
        bool  GetCallConv( ref uint8_t  callConv ) { return  false; }
        bool  GetParamCount( ref uint16_t  count ) { return  false; }
        bool  GetParamList( ref TypeIndex  index ) { return  false; }
        bool  GetClass( ref TypeIndex  index ) { return  false; }
        bool  GetThis( ref TypeIndex  index ) { return  false; }
        bool  GetThisAdjust( ref int32_t  adjust ) { return  false; }
        bool  GetOemId( ref uint32_t  oemId ) { return  false; }
        bool  GetOemSymbolId( ref uint32_t  oemSymId ) { return  false; }
        bool  GetTypes( ref std.vector!(TypeIndex)  indexes ) { return  false; }
        bool  GetAttribute( ref uint16_t  attr ) { return  false; }
        bool  GetVBaseOffset( ref uint32_t  offset ) { return  false; }
        bool  GetVTableDescriptor( uint32_t  index, ref uint8_t  desc ) { return  false; }
        bool  GetMod( ref uint16_t  mod ) { return  false; }
}
    };
    ...
    public:
        this( uint32_t  regIdx ) { mIndex = ( regIdx );}

        SymTag  GetSymTag() { return  SymTagData; }
        bool  GetName( ref SymString  name ) 
        {
        ...
        bool  GetRegister( ref uint32_t  reg )
        {
            reg = regDesc[ mIndex ].cvreg;
            return  true;
        }
        bool  GetLength( ref uint32_t  length )
        {
            length = MagoEE.TypeBasic.GetTypeSize( regDesc[mIndex].type );
            return  true;
        }
        bool  GetDataKind( ref DataKind  dataKind ) { dataKind = DataIsConstant; return  true; }
        bool  GetLocation( ref LocationType  locType ) { locType = LocIsEnregistered; return  true; }
    };

    static  std.map!(std.string, uint32_t) mapNameToRegIndex;

    static  bool   /* SYNTAX ERROR: (1958): expression expected, not [ */ 
    
           /* SYNTAX ERROR: (1960): expected <identifier> instead of sizeof */  /* SYNTAX ERROR: (1960): expected ; instead of ++ */ 
             

         /* SYNTAX ERROR: (1963): expected <identifier> instead of return */ 
     /* SYNTAX ERROR: unexpected trailing } */ } /* SYNTAX ERROR: (1964): expected <identifier> instead of ( */ 

    MagoST.SymInfoData  registerInfoData;

    this.RegisterCVDecl( ExprContext* symStore, uint32_t  regIndex )
    { GeneralCVDecl = ( symStore, registerInfoData, cast(ISymbolInfo) &registerInfoData );
    ...
    this.~RegisterCVDecl()
    {
        delete cast(RegisterSymbolInfo) mSymInfo;
    }
    ...
    public:
        this( uint32_t  cnt) { mCount = ( cnt );}

        SymTag  GetSymTag() { return  SymTagData; }
        bool  GetOffset( ref int  offset ) { offset = 0; return  true; }
        bool  GetDataKind( ref DataKind  dataKind ) { dataKind = DataIsConstant; return  true; }
        bool  GetLocation( ref LocationType  locType ) { locType = LocIsThisRel; return  true; }
    };

    this.VTableCVDecl( ExprContext* symStore, uint32_t  count, MagoEE.Type* type )
    { GeneralCVDecl = ( symStore, registerInfoData, cast(ISymbolInfo) &registerInfoData );
        // CVDecl asumes registerInfoData is an instance of mSymInfo
    ...
    this.~VTableCVDecl()
    {
        delete cast(VTableSymbolInfo) mSymInfo;
    }

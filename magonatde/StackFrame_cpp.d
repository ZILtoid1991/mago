...
namespace  Mago
{
    ...
    {   mPC = ( 0 );
            mPtrSize = ( 0 );
        memset( &mFuncSH, 0,  mFuncSH.sizeof );
    }
       ...
       IDebugCodeContext2** ppCodeCxt )
    {
        ...
        return  codeContext.QueryInterface( __uuidof( IDebugCodeContext2 ), cast(void **) ppCodeCxt );
    }
       ...
       IDebugDocumentContext2** ppCxt )
    {
        ...
        return  docContext.QueryInterface( __uuidof( IDebugDocumentContext2 ), cast(void **) ppCxt );
    }
       ...
       FRAMEINFO*      pFrameInfo )
    {
        ...
        {
            hr = QueryInterface( __uuidof( IDebugStackFrame2 ), cast(void **) &pFrameInfo.m_pFrame );
            _ASSERT( hr == S_OK );
        ...
        {
            if ( mModule.Get() !is  null )
            {
                hr = mModule.QueryInterface( __uuidof( IDebugModule2 ), cast(void **) &pFrameInfo.m_pModule );
                _ASSERT( hr == S_OK );
...
static if(0) {
        ...
        if ( (dwFieldSpec & FIF_MODULE) != 0 )
        {
            if ( mModule !is  null )
            {
                ...
                if ( pFrameInfo.m_bstrModule !is  null )
    ...
    HRESULT  StackFrame.MakeExprContext()
    {
        ...
        if ( mExprContext  is  null )
        {
            GuardedArea  guard = GuardedArea( mExprContextGuard );
       ...
       IDebugThread2** ppThread )
    {
        ...
        return  mThread.QueryInterface( __uuidof( IDebugThread2 ), cast(void **) ppThread );
    }
    ...
    HRESULT  StackFrame.GetLineInfo( ref LineInfo  info )
    {
        HRESULT  hr = S_OK;
        RefPtr<MagoST.ISession>    session;
        ...
        info.Address = cast(Address64) session.GetVAFromSecOffset( line.Section, line.Offset );
        if( !info.Address )
            return  E_FAIL;
        ...
        ref CString  fullName )
    {
        ...
        if ( (flags & FIF_FUNCNAME_LINES) != 0 )
        {
            ...
            if ( SUCCEEDED( GetLineInfo( line ) ) )
            {
                ...
                fullName.AppendFormat( " %s %u"w, lineStr, lineNumber );
            }
        }
        ...
        if ( !hasLineInfo )
        {
            ...
            baseAddr = cast(Address64) session.GetVAFromSecOffset( sec, offset );
            if ( baseAddr == 0 )
                return  E_FAIL;
        }
        ...
        if ( ((flags & FIF_FUNCNAME_OFFSET) != 0) && (mPC != baseAddr) )
        {
            ...
            fullName.AppendFormat( " + 0x%x %s"w, cast(uint32_t) (mPC - baseAddr), bytesStr );
        }
        ...
        while ( session.NextSymbol( funcScope, childSH, ~0 ) )
        {
            ...
            if ( (flags & FIF_FUNCNAME_ARGS_TYPES) != 0 )
            {
                if ( decl.GetType( type.Ref() ) )
                {
                    ...
                    outputStr.AppendFormat( " %.*s"w, typeStr.size(), typeStr.c_str() );
                }
            }

            if ( (flags & FIF_FUNCNAME_ARGS_NAMES) != 0 )
            {
                outputStr.AppendFormat( " %s"w, decl.GetName() );
            }
            ...
            if ( (flags & FIF_FUNCNAME_ARGS_VALUES) != 0 )
            {
                ...
                if ( hr == S_OK )
                {
                    ...
                    if ( hr == S_OK )
                    {
                        outputStr.AppendFormat( " = %.*s"w, valueStr.size(), valueStr.c_str() );
                    }
                }
            }
    ...
    HRESULT  StackFrame.FindFunction()
    {
        ...
        // already found
        if ( memcmp( &mFuncSH, &symHandle,  mFuncSH.sizeof ) != 0 )
            return  S_OK;

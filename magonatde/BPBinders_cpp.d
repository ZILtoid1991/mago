...
namespace  Mago
{
    ...
    this.BPCodeFileLineBinder(
        IDebugBreakpointRequest2* request )
    {   mReqLineStart = ( 0 );
            mReqLineEnd = ( 0 );
        _ASSERT( request !is  null );
    ...
    void  BPCodeFileLineBinder.Bind( Module* mod, ModuleBinding* binding, BPBoundBPMaker* maker, ref Error  err )
    {
        HRESULT                  hr = S_OK;
        ...
        ref Error  err )
    {
        ...
        for( std.list<MagoST.LineNumber>.iterator  /* SYNTAX ERROR: (86): expected ; instead of it */  it = lines.begin(); it != lines.end(); ++it )
        {
            PutLineError( err );
    ...
    void  BPCodeFileLineBinder.PutDocError( ref Error  err )
    {
        err.PutError( BPET_TYPE_WARNING, BPET_SEV_LOW, IDS_NO_SYMS_FOR_DOC );
    }

    void  BPCodeFileLineBinder.PutLineError( ref Error  err )
    {
    ...
    this.BPCodeAddressBinder( IDebugBreakpointRequest2* request )
    {   mAddress = ( 0 );
        _ASSERT( request !is  null );
    ...
    void  BPCodeAddressBinder.Bind( Module* mod, ModuleBinding* binding, BPBoundBPMaker* maker, ref Error  err )
    {

...
namespace  Mago
{
    ...
    {   mBlockCount = ( blockCount );
        ...
        _ASSERT( blocks !is  null );
        _ASSERT( blocks[0] !is  null );
        _ASSERT( (blockCount == 1) || (blocks[1] !is  null) );
        ...
        ud_init( &mDisasm );
        ud_set_mode( &mDisasm, cast(uint8_t) (ptrSize * 8) );
        ud_set_syntax( &mDisasm, UD_SYN_INTEL );

        if ( disasmStream !is  null )
        {
    ...
    {
        IDebugDisassemblyStream2* dds = cast(IDebugDisassemblyStream2*) ud.sym_context;
        ...
        if( FAILED( hr ) || info.bstrFunction  is  null )
            return  0;
        ...
        ud.insn_fill += sprintf( cast(char *) ud.insn_buffer + ud.insn_fill, " (0x%I64x)", addr );
    ...
    uint32_t  InstReader.TruncateBeforeAnchor()
    {
        ...
        if ( mCurAddr < mAnchorAddr && limit > mAnchorAddr )
        {
            ...
            instLen = cast(uint32_t) (mAnchorAddr - mCurAddr);
        }
    ...
    bool  InstReader.ReadByte( ref BYTE  dataByte )
    {
        uint32_t     instBufLen = 0;
        BYTE*       instBuf = null;
        ...
        if ( (instBuf  is  null) || (instBufLen == 0) )
            return  false;
    ...
    BYTE* InstReader.GetInstBuffer( ref uint32_t  length )
    {
        ...
        uint32_t     maxInstLen = cast(uint32_t) (mEndAddr - mCurAddr);
        uint32_t     virtPos = cast(uint32_t) (mCurAddr - mBlocks[0].Address);
        ...
        {
            size_t   sizeOnLeft = cast(size_t) (mBlocks[0].GetLimit() - mCurAddr);
    ...
    HRESULT  InstCache.LoadBlocks( Address64  addr, int  instAway, ref int  instAwayAvail )
    {
        ...
        if ( (leftBase < anchorBase) 
            && (instAway < (cast(intptr_t) (anchorBase - addr) / MaxInstructionSize)) )
        {
            sideBase = leftBase;
        }
        else  if ( (rightLimit > anchorBase) 
            && (instAway > (cast(intptr_t) (rightBase - addr) / MaxInstructionSize)) )
        {
            sideBase = rightBase;
        }
        ...
        if ( instAway < 0 )
        {
            instAwayAvail = cast(intptr_t) (baseOnLeft - addr) / MaxInstructionSize;
            instAwayAvail = std.max( instAwayAvail, instAway );
        }
        else
        {
            instAwayAvail = cast(intptr_t) (limitOnRight - addr) / MaxInstructionSize;
    ...
    void  InstCache.MapInstData( Address64  anchorAddr )
    {
        ...
        if ( anchorBlock  is  null )
            return;
        if ( (leftBlock !is  null) && (leftBlock.Address >= anchorBlock.Address) )
            leftBlock = null;
        if ( (rightBlock !is  null) && (rightBlock.Address <= anchorBlock.Address) )
            rightBlock = null;

        if ( leftBlock !is  null )
        {
            sideBlock = leftBlock;
            blocks[0] = leftBlock;
            blocks[1] = anchorBlock;
        }
        else  if ( rightBlock !is  null )
        {
        ...
        else  if ( (sideBlock !is  null) && (sideBlock.State == BlockState_Loaded) )
        {
            // do 1 whole map in 1 run
            blocks[0] = sideBlock;
        ...
        uint32_t  blockCount, InstBlock** blocks, Address64  startAddr, Address64  endAddr )
    {
        ...
        InstReader   reader = InstReader( blockCount, blocks, startAddr, endAddr, mAnchorAddr, mPtrSize, null );
        uint32_t     instLen = 0;
        uint32_t     virtPos = cast(uint32_t) (startAddr - blocks[0].Address);

        if ( blockCount == 1 )
        {
            uint32_t     size = cast(uint32_t) (endAddr - startAddr);
        ...
        else
        {
            ...
            uint32_t     size = cast(uint32_t) (blocks[1].Address - startAddr);

            memset( &blocks[0].Map[virtPos], 0, size );

            size = cast(uint32_t) (endAddr - blocks[1].Address);
        ...
        for ( instLen = reader.Decode(); instLen != 0; instLen = reader.Decode() )
        {
            ...
            if ( ud.mnemonic != UD_Iinvalid )
            {
                if ( virtPos < InstBlock.BlockSize )
                    blocks[0].Map[ pos ] = cast(BYTE) instLen;
                else  if ( blocks[1] !is  null )
                    blocks[1].Map[ pos ] = cast(BYTE) instLen;
    ...
    InstBlock* InstCache.GetBlockContaining( Address64  addr )
    {
        ...
        _ASSERT( i < cast(int) _countof( mBlockCache ) );

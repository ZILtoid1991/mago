...
alias  
     ...
     /* SYNTAX ERROR: (25): expected <identifier> instead of & */  
...
namespace  Mago
{
    ...
    this.EventCallback( Engine* engine )
    {   mRefCount = ( 0 );
            mEngine = ( engine );
    }
    ...
    HRESULT  EventCallback.SendEvent( EventBase* eventBase, Program* program, Thread* thread )
    {
        ...
        hr = mEngine.QueryInterface( __uuidof( IDebugEngine2 ), cast(void **) &ad7Engine );
        _ASSERT( hr == S_OK );

        hr = program.QueryInterface( __uuidof( IDebugProgram2 ), cast(void **) &ad7Prog );
        _ASSERT( hr == S_OK );

        if ( thread !is  null )
        {
            hr = thread.QueryInterface( __uuidof( IDebugThread2 ), cast(void **) &ad7Thread );
            _ASSERT( hr == S_OK );
        }
    ...
    void  EventCallback.OnModuleLoadInternal( DWORD  uniquePid, ICoreModule* coreModule )
    {
        ...
        hr = mod.QueryInterface( __uuidof( IDebugModule2 ), cast(void **) &mod2 );
        if ( FAILED( hr ) )
            return;
        ...
        hr = mod.QueryInterface( __uuidof( IDebugModule3 ), cast(void **) &mod3 );
        if ( FAILED( hr ) )
            return;
        ...
        if( flags & MIF_SYMBOLS_LOADED )
            msg.Append( " loaded, symbols found."w);
        else
             msg.Append( " loaded, no symbols."w);
    ...
    bool  FindGlobalSymbolAddress( Module* mainMod, const(char) * symbol, ref Address64  symaddr )
    {
        ...
        symaddr = cast(Address64) addr;
        return  true;
    }

    bool  FindUserEntryPoint( Module* mainMod, ref Address64  entryPoint )
    {
        ...
        DWORD  uniquePid, DWORD  threadId, bool  firstChance, const  EXCEPTION_RECORD64* exceptRec )
    {
        ...
        if ( found )
        {
            ...
            {
                wchar_t  name[256] = ""w;
                _swprintf_p( name, _countof( name ), "0x%08x: %s"w, event.GetCode(), cast(BSTR) info.bstrExceptionName );
        ...
        Program* prog, Thread* thread, Address64  address, bool  embedded )
    {
        ...
        else
        {
            ...
            for ( std.vector< BPCookie >.iterator  /* SYNTAX ERROR: (489): expected ; instead of it */  it = iter.begin(); it != iter.end(); it++ )
            {
            ...
            if ( stoppingBPs > 0 )
            {
                ...
                int  i = 0;
                for ( std.vector< BPCookie >.iterator  /* SYNTAX ERROR: (512): expected ; instead of it */  it = iter.begin(); it != iter.end(); it++ )
                {
                    if ( *it != EntryPointCookie )
                    {
                        IDebugBoundBreakpoint2* bp = cast(IDebugBoundBreakpoint2*) *it;
    ...
    ProbeRunMode  EventCallback.OnCallProbe( 
        DWORD  uniquePid, uint32_t  threadId, Address64  address, ref AddressRange64  thunkRange )
    {
    ...
    bool  EventCallback.FindThunk( 
        MagoST.ISession* session, uint16_t  section, uint32_t  offset, ref AddressRange64  thunkRange )
    {
        HRESULT  hr = S_OK;
        ...
        if ( hr == S_OK )
        {
            ...
            if ( hr == S_OK )
            {
                if ( symInfo.GetSymTag() == MagoST.SymTagThunk )
                {
                    ...
                    if ( addr == 0 )
                        return  false;
                    thunkRange.Begin = cast(Address64) addr;
                    thunkRange.End = cast(Address64) addr + length - 1;
                    return  true;
                }
            }
        }

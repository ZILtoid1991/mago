...
const wchar ReplacementChar16 = 0xFFFD;
...
{
    assert((utf32Len == 0) || (utf32Str !is null));
    ...
    if (utf8Len < 0)
        utf8Len = cast(int) strlen(utf8Str) + 1; // include terminator in length to convert
    ...
    while (pos < utf8Len)
    {
        ...
        else
        {
            ...
            if ((c == 0xC0) || (c == 0xC1) || (c == 0xFE) || (c == 0xFF)
                    || ((c >= 0xF5) && (c <= 0xF7))) // TODO: actually F8...FD are also restricted
                    {
...
int Utf8To16(const(char)* utf8Str, int utf8Len, wchar_t* utf16Str, int utf16Len)
{
    ...
    return MultiByteToWideChar(CP_UTF8, Flags, utf8Str, utf8Len, utf16Str, utf16Len);
}
...
int Utf16To8(const(wchar_t)* utf16Str, int utf16Len, char* utf8Str, int utf8Len)
{
    ...
    return WideCharToMultiByte(CP_UTF8, Flags, utf16Str, utf16Len, utf8Str, utf8Len, null, null);
}
...
int Utf16To32(const(wchar_t)* utf16Str, int utf16Len, dchar_t* utf32Str, int utf32Len)
{
    ...
    if (utf16Len < 0)
        utf16Len = cast(int) wcslen(utf16Str) + 1; // include terminator in length to convert
...
int Utf32To16(bool ignoreErrors, const dchar_t* utf32Str, int utf32Len,
        wchar_t* utf16Str, int utf16Len, ref bool truncated)
{
    assert((utf16Len == 0) || (utf16Str !is null));
    ...
    truncated = false;
    if (utf32Len < 0)
        utf32Len = cast(int) dcslen(utf32Str) + 1; // include terminator in length to convert
    ...
    while (pos < utf32Len)
    {
        ...
        if (c > 0xFFFF)
        {
            ...
            wchar_t w1 = cast(wchar_t)(c2 >> 10);
            wchar_t w2 = cast(wchar_t)(c2 & 0x3FF);
            w1 |= 0xD800;
            w2 |= 0xDC00;
...
void AppendChar32(ref wstring str, dchar_t c)
{
    if (c > 0xFFFF)
    {
        dchar_t c2 = c - 0x10000;
    ...
    {
        str ~= 1, cast(wchar_t) c;
    }
}

void AppendChar32(ref char[] str, dchar_t c)
{
    int n = 0;
    uint8_t b = 0;
    ...
    uint8_t decor = cast(uint8_t)((cast(int) 0xFFFFFFC0) >> (n - 1));
    b = cast(uint8_t)((c & 0x1F) >> (n - 1));
    b |= decor;
